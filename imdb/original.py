...
class Net(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        elif self.model_type in ['lstm-light', 'LSTM-light',
        ...
        elif self.model_type in ['lstm-light-onedirect', 'LSTM-light-onedirect',
    ...
    def forward_sub(self, embeded_words):
        ...
        if self.model_type in ['rnn', 'lstm', 'gru']:
            ...
            embeded_words = self.encoder_word(embeded_words.view(-1,
                                    self.max_num_words,
                                    self.embedding_dim))[0] # batch * sent, word-dim, hidden-dim

            encoded_sent = torch.mean(embeded_words,
                                      -2).view(-1,
                                               self.max_num_sents,
                                               self.num_rnn_out_size * self.mul_bidirect)
            ...
            encoded_review = self.dense(embeded_words.contiguous().\
                                        view(embeded_words.size(0), -1, 1).squeeze(-1))
            pred = F.log_softmax(encoded_review, dim = -1)

        elif self.model_type in ['cnn', 'CNN']:

            encoded_sent = self.encoder_word(embeded_words.view(-1,
                                    1, self.max_num_words * self.embedding_dim))
...
def test(args, model, device, test_loader, outfile = False, compare = True, **kwargs):
    ...
    outfile_path = os.path.join(args.data_dir, 'processed') # if you want to change this directory, you also need to change 'path_to_file' object in 'data_loader.py'
    outmode = 'VALID'
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(test_loader):
            ...
            target = batch.label.view(-1) - 2
            fname = batch.fname
            target_pred = batch.label_pred.view(-1)
            ...
            correct += pred.eq(target.view_as(pred)).sum().item()
            correct_pre += target_pred.eq(target.view_as(target_pred)).sum().item()
        ...
        if outfile:
            ...
            print(outmode, 'set: from last prediction, Accuracy: {}/{} ({:.0f}%)'.\
                  format(correct_pre, total_size, 100. * correct_pre / total_size))
            print(outmode, 'set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)'.\
                  format(test_loss, correct, total_size, 100. * correct / total_size))
...
def main():
    ...
    parser.add_argument('--dataset', default = 'imdb', type = str,
                        help='dataset name: imdb, mnist, mimic')
    ...
    parser.add_argument('--model_type', default = 'lstm-light', type = str,
                        help = 'cnn, lstm, rnn, gru, lstm-light')
    ...
    args.max_num_words = data_loader['max_num_words'] # default 100
    args.max_num_sents = data_loader['max_num_sents'] # default 15    
