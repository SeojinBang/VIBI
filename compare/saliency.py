...
def parse_args():
    ...
    parser.add_argument('--dataset', default = 'imdb', type = str, help='dataset name: imdb, imdb, mnist')
    ...
    parser.add_argument('--model_name', default = 'original_rnn.ckpt', type = str, help = 'if train is True, model name to be saved, otherwise model name to be loaded')
...
#                        _, index_chunk = torch.abs(grad_chunk.view(grad_size[0], grad_size[1], -1) *
#                                                   input.view()
#                                                   ).topk(k, dim = -1)
...
#                        UnknownModelError()
                ...
                input_embeded = Variable(model.embedding_layer(input), requires_grad = True)
                ...
                if args.method == 'saliency':
                    ...
                    grad_ft = SmoothGrad(pretrained_model = model.forward_sub, 
                                         is_cuda = args.cuda,
                                         n_samples = 10,
                                         magnitude = False)
                    grad = grad_ft(input_embeded, index = None)
        ...
        for outidx in range(num_labels):
            ...
            for i in range(data.size(0)):
                ...
                if 'mnist' in args.dataset:
                    ...
                    if args.method == 'saliency':
                        grad, = torch.autograd.grad(output[0][outidx], input_embeded, retain_graph = True)
